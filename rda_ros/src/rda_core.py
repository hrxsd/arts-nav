...
class rda_core:
    def __init__(self) -> None:
        ...
        self.goal_reached = False      # 目标是否达成
        self.goal_published = False    # 目标是否已发布到movebase
    ...
    def control(self):
        ...
        while not rospy.is_shutdown():
            ...
            # 处理新目标 - 仅发布一次给move_base
            if self.new_goal_received and not self.goal_published and self.current_goal is not None:
                ...
                rospy.loginfo("已将目标发布到move_base")
            ...
            if self.rda_opt.no_ref_path() or len(self.ref_path_list) == 0:
                rospy.loginfo_throttle(
                    1, "waiting for reference path from move_base"
                )
                # 发布零速度命令，确保机器人不会移动
                zero_vel = Twist()
            ...
            if info["arrive"] and not self.goal_reached:  # 仅触发一次
                if self.loop:
                    ...
                    self.goal_reached = True # 目标到达时设置标志
    ...
    def global_path_callback(self, path):
        ...
        if len(self.ref_path_list) == 0:
            rospy.loginfo_throttle(
                1,
                "No waypoints in global path, waiting for new path"
            )
            return
        
        rospy.loginfo_throttle(0.1, "reference path updated from global planner")
        self.rda_opt.update_ref_path(self.ref_path_list)

        # 新路径到达后更新状态
        self.goal_published = True  # 已经有全局路径，不需要再发布目标
    ...
    def goal_callback(self, goal):
        ...
        rospy.loginfo(f"接收到新rda goal {self.goal}")
        ...
        # 不再在这里生成参考路径
        # 而是发布目标点到 move_base，让它生成全局路径
        # 这需要你创建一个发布者来发布目标点到 move_base
        
        # # 如果你想直接将目标发送给 move_base
        # goal_msg = PoseStamped()
        # goal_msg.header.frame_id = self.target_frame
        # goal_msg.header.stamp = rospy.Time.now()
        ...
        # # 添加一个新的发布者
        # if not hasattr(self, 'move_base_goal_pub'):
        #     self.move_base_goal_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=1)
