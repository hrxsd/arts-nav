...
#include "/home/nuc1003a/ARTS_TEST/src/sr_pkg/include/qtts.h"
...
void xfcallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  std::cout<<"WALLE: "<<msg->data.c_str()<<std::endl;
  text = msg->data.c_str(); 
  /* 文本合成 */
  printf("开始合成 ...\n");
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = adc23377, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
    ...
    ros::NodeHandle n;
    ros::Subscriber sub =n.subscribe("response",1000,xfcallback);
    ros::spin();
