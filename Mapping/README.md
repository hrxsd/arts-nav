...
4. [FAST-LIO-LOCALIZATION](https://github.com/HViktorTsoi/mapping_LOCALIZATION): The integration of FAST-LIO with **Re-localization** function module.
...
[FAST-LIO2: Fast Direct LiDAR-inertial Odometry](doc/mapping_2.pdf)
...
```
    cd ~/$A_ROS_DIR$/src
    git clone https://github.com/hku-mars/mapping.git
    cd mapping
...
```
    cd ~/$mapping_ROS_DIR$
    source devel/setup.bash
    roslaunch mapping mapping_avia.launch
...
```
    cd ~/$mapping_ROS_DIR$
    source devel/setup.bash
    roslaunch mapping mapping_velodyne.launch
```
...
Set ``` pcd_save_enable ``` in launchfile to ``` 1 ```. All the scans (in global frame) will be accumulated and saved to the file ``` mapping/PCD/scans.pcd ``` after the FAST-LIO is terminated. ```pcl_viewer scans.pcd``` can visualize the point clouds.
