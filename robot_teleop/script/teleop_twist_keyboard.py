...
class PublishThread(threading.Thread):
    ...
    def get_current_pose(self):
        ...
        tf_pose = self.tfBuffer.lookup_transform(
        # "map", "base_link", rospy.Time(), timeout=rospy.Duration(2)
        "camera_init", "body", rospy.Time(), timeout=rospy.Duration(2)
        )
        pose.x = tf_pose.transform.translation.x
        pose.y = tf_pose.transform.translation.y
        quaternion = (
        tf_pose.transform.rotation.x,
        tf_pose.transform.rotation.y,
        tf_pose.transform.rotation.z,
        tf_pose.transform.rotation.w)
        roll_, pitch_, yaw_ = euler_from_quaternion(quaternion)
        pose.theta = yaw_
        self.pose = pose
...
if __name__=="__main__":
    ...
    dir_json = os.path.join(dir_pa,"ARTS_test/src/Mapping/config/target_map.json")
    # print("################# ", dir_pa)
    try:
        pub_thread.wait_for_subscribers()
        pub_thread.update(x, y, z, th, speed, turn)
        
        pose = pub_thread.get_current_pose()
        print(pose)
        print(msg)
